  ...
  return (
    <>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
        <div className={`pointer-events-auto backdrop-blur-xl shadow-2xl border rounded-3xl p-2 flex items-center gap-2 transition-colors duration-300 ${
          ...
          {mainTools.map((t) => (
            <div key={t.id} className="relative">
              <button
                ...
                title={t.label}
              >
                <t.icon size={22} strokeWidth={(activeTool === t.id || (t.id === 'shapes' && shapeTools.some(s => s.id === activeTool))) ? 2.5 : 2} />
              ...
              {t.id === 'shapes' && showShapesMenu && (
                <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-4 p-3 rounded-2xl shadow-2xl border backdrop-blur-xl animate-in fade-in slide-in-from-bottom-2 duration-200 min-w-[240px] ${
          ...
          {/* Divider */}
          <div className={`w-px h-8 bg-gradient-to-b from-transparent via-gray-300 to-transparent mx-1 ${isDarkMode ? "via-slate-700" : "via-gray-300"}`} />
          ...
          <div className="relative">
            <button
              ...
              className={`group relative p-3.5 rounded-2xl transition-all duration-300 hover:scale-105 ${
                isDarkMode ? "hover:bg-slate-800" : "hover:bg-gray-100"
              }`}
              title="Color Picker"
            >
              <div className="relative">
                <Palette size={22} className={isDarkMode ? "text-slate-300" : "text-gray-600"} strokeWidth={2} />
                <div 
                  className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 shadow-sm ${isDarkMode ? "border-slate-800" : "border-white"}`}
          ...
          >
            <Grid3X3 size={22} strokeWidth={showGrid ? 2.5 : 2} />
            {/* Tooltip */}
            <span className="absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
